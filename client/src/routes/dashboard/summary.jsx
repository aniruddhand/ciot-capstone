...
export default function SummaryPage() {
    ...
    useEffect(() => {
        ...
        const cachedStatus = sessionStorage.getItem('wlgwStatus');
        if (cachedStatus) {
        ...
        fetch('/api/status').then(response => {
            if (response.status >= 200 && response.status < 400) {
                response.json().then(body => {
                   setStatus(body);
                   sessionStorage.setItem('wlgwStatus', JSON.stringify(body));
                });
            } else {
                        ...
                        <div className='row g-0 h-100'>
                            ...
                            <div className='col-md-8'>
                                <div className='card-body'>
                                    ...
                                    <p className='card-text'>The sump tank has <span className='fw-bold'>{status.wlgw.volume}</span> liters of water.</p>
                                    <p className='card-text'><small className='text-muted'>Last updated {ago(new Date(status.wlgw.timestamp))}</small></p>
                                </div>
                            </div>
                        </div>
